use statcan_rs::{GenericCKANDriver, CKANClient, DataHandler};

#[tokio::test]
async fn test_ckan_ping() {
    let driver = GenericCKANDriver::new("https://open.canada.ca/data").expect("Failed to create driver");
...
async fn test_job_bank_package() {
    ...
    match driver.get_package_metadata(job_bank_id).await {
        Ok(meta) => {
            ...
            if let Some(res) = meta.resources.iter().find(|r| r.format.as_deref() == Some("CSV")) {
                ...
                match driver.get_resource_handler(&res.id).await {
                    Ok(handler) => {
                         ...
                         if let statcan_rs::DataHandler::DatastoreQuery(rid) = handler {
                             ...
                             match driver.query_datastore(&sql).await {
                                 Ok(records) => {
                                     println!("SQL Query Success: Retrieved {} records", records.len());
