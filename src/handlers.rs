...
pub async fn handle_discover_datasets<C: StatCanClientTrait, O: CKANClient>(
    ...
    if let Ok(packages) = od_res {
        for pkg in packages.into_iter().take(limit) {
            // Add suggested best resource ID if available
            let mut best_resource_id = None;
            if let Some(best) = crate::data_helpers::select_best_resource(&pkg.resources) {
                best_resource_id = Some(best.id.clone());
            }

            unified_results.push(json!({
                "source": "OpenData",
                "id": best_resource_id.unwrap_or_else(|| pkg.id.clone()),
                "title": pkg.title,
                "type": "resource",
...
pub async fn handle_inspect_dataset<C: StatCanClientTrait, O: CKANClient>(
    ...
    if is_statcan {
        ...
        let schema = od_client.get_resource_schema(dataset_id).await.map_err(|e| {
...
pub async fn handle_query_data<C: StatCanClientTrait, O: CKANClient>(
    ...
    if is_statcan {
        ...
        if let Some(f) = filters {
            new_args.as_object_mut().unwrap().insert("filters".to_string(), json!(f));
            if let Some(geo) = f.get("Geography").and_then(|v| v.as_str()) {
                new_args.as_object_mut().unwrap().insert("geo".to_string(), json!(geo));
            }
        }

        if let Some(dr) = date_range {
            if let Some(last_n) = dr.get("last_n_periods").and_then(|v| v.as_u64()) {
                new_args.as_object_mut().unwrap().insert("recent_months".to_string(), json!(last_n));
            }
        }
    ...
    match method {
        ...
        "tools/call" => {
            ...
            match name {
                ...
                // Keep old ones for backward compatibility just in case, but they are deprecated
