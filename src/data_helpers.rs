...
pub fn score_cube_title_match(title: &str, query_lower: &str, query_terms: &[&str]) -> f64 {
    let title_lower = title.to_lowercase();

    let is_exact = title_lower.contains(query_lower);

    let has_all_terms = query_terms.iter().all(|term| title_lower.contains(*term));
...
mod tests {
    ...
    fn test_score_cube_title_match() {
        ...
        let exact_score = score_cube_title_match("Labour force characteristics by province", "labour force", &["labour", "force"]);
        assert!(exact_score > 2.0); // 2.0 (exact) + similarity

        // All terms match + jaro winkler
        let terms_score = score_cube_title_match("Labour force characteristics by province", "force labour", &["force", "labour"]);
        assert!(terms_score > 1.0); // 1.0 (all terms) + similarity
        assert!(terms_score < 2.0);

        // Fuzzy match
        let fuzzy_score = score_cube_title_match("Labour force characteristics by province", "labor forc", &["labor", "forc"]);
